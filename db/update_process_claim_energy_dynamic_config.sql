-- 🟢 更新 process_claim_energy 函数：支持动态能量配置
-- 目的：允许通过参数传递能量奖励配置，实现动态调整
-- 执行时间: 2026-01-06
-- 方案：方案A - 通过参数传递（推荐）
...
DECLARE
  test_result jsonb;
BEGIN
  RAISE NOTICE '开始测试 process_claim_energy 函数...';
  
  -- 测试场景：新用户首次领取，有推荐人
  -- 预期：用户获得 1 能量，推荐人获得 3 能量
  test_result := process_claim_energy(
    '0xtest_tx_hash_' || floor(random() * 1000000)::text,
    '0xtest_user_' || floor(random() * 1000000)::text,
    '0xtest_referrer_' || floor(random() * 1000000)::text,
