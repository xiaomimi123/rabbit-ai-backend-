...
### 1. **withdrawals 表 - 提现状态变更没有独立审计** 🔴 P1

**当前状况**:
- ✅ 提现会触发 `users` 表变更，从而记录到 `users_audit_log`
- ❌ 但提现记录本身的状态变更（Pending → Completed/Rejected）没有独立审计日志
...
### 3. **system_config 表 - 系统配置变更没有审计** 🟡 P2
...
### 4. **auto_payout_config 表 - 自动放款配置变更没有审计** 🟡 P2
